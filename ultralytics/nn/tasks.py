...
class DetectionModel(BaseModel):
    ...
    def init_criterion(self):
        ...
        return E2EDetectLoss(self) if getattr(self, "end2end", False) else v8DetectionSimLoss(self)
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # --------------GOLD-YOLO--------------
            ...
            c2 = sum(ch[x] for x in f)
        elif m in {Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect,Detect_featureloss}:
            args.append([ch[x] for x in f])
            if m is Segment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, Segment, Pose, OBB, Detect_featureloss}:
                m.legacy = legacy
